...
with xlsxwriter.Workbook('Overlaps_of_Upregulated_Genes.xlsx', {'nan_inf_to_errors': True}) as workbook:
# Create new worksheet and print keys at the top
    worksheet = workbook.add_worksheet()
    d = pd.read_excel('/Users/kevinvela/Dropbox/Imperial_College_London/Year_3/FYP/Dissertation/Kev/Upregulated_Genes.xlsx', sheet_name = 1)
    keyss = d.keys()
    for key in range(len(keyss)-1):
        worksheet.write( 0, key, keyss[key])
    for key in range(len(keyss)-1):
        worksheet.write( 0, key+16, keyss[key])

# We create an index to count a new row for every new pair
    index = 1
    for sheet in range(7):
        df = pd.read_excel('/Users/kevinvela/Dropbox/Imperial_College_London/Year_3/FYP/Dissertation/Kev/Upregulated_Genes.xlsx', sheet_name = sheet)

        for sheet2 in range(7):
#if sheet is same skip
            if(sheet == sheet2):
                continue

            df2 = pd.read_excel('/Users/kevinvela/Dropbox/Imperial_College_London/Year_3/FYP/Dissertation/Kev/Upregulated_Genes.xlsx', sheet_name=sheet2)
            keys = df.keys()
            keys2 = df2.keys()
